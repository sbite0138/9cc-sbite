...
try 4 "int *x; int main(){ return sizeof(sizeof(x));}"
try 2 "int *x; int main(){ calloc4(&x,1,2,3,4);return  *(x+1);}"
try 34 "int a[30];int main(){  *(a+1)=2;*a=32;return *(a+1)+*a;}"
try 42 "int *a[30];int y ;int main(){ int x;x=32;y=10; *(a+1)=&x;*a=&y;return **(a+1)+**a;}"
try 3 "int a[2];int main(){*a = 1;*(a + 1) = 2;int *p;p = a;return *p + *(p + 1) ;}"
try 34 "int a[30];int main(){a[3]=2;a[0]=32; return a[3]+a[0];}"
try 13 "int i;int main(){int a[30]; for (i=0;i<30;i=i+1){a[i]=i*i;} return a[3]+a[2]; }"
...
try 14 'int main(){int a[10][10];a[3][3]=3;a[9][2]=11;return a[3][3]+a[9][2];}'
echo OK